			...
			<section v-if="open" class="update-center-popover">
					...
					<div class="update-summary">
						<strong>{{
							loading
								? 'Проверяем сборки…'
								: instanceUpdates
									? `Обновлений сборок: ${instanceUpdates}`
									: 'Сборки актуальны'
						}}</strong>
						<span>Обновления устанавливаются только по вашей команде.</span>
					</div>
					<div class="update-instance-list">
						<div v-for="item in pendingItems" :key="item.path" class="update-instance-row">
							<div class="instance-copy">
								<strong>{{ item.name }}</strong
								><span>{{ item.gameVersion }} · {{ item.loader }}</span>
							</div>
						...
						<div v-if="!loading && pendingItems.length === 0" class="update-empty">
							{{ items.length ? 'Все сборки актуальны.' : 'Сборки пока не созданы.' }}
						</div>
					</div>
					<footer>
						<button type="button" :disabled="loading || checkingState" @click="scan">
							<RefreshCwIcon /> Проверить снова
						</button>
						<button
							type="button"
							class="update-all"
							:disabled="instanceUpdates === 0 || updatingAll"
							@click="updateAllInstances"
						>
				...
				<div v-else class="update-tab-panel release-notes-panel">
					<article v-for="note in releaseNotes" :key="note.version" class="release-note">
						...
						<section v-if="note.highlights.length">
							<strong>Новое</strong>
							<ul>
								<li v-for="item in note.highlights" :key="item">{{ item }}</li>
							</ul>
						</section>
						<section v-if="note.fixes.length">
							<strong>Исправления</strong>
							<ul>
								<li v-for="item in note.fixes" :key="item">{{ item }}</li>
							</ul>
						</section>
						<section v-if="note.improvements.length">
					...
					>
						<div>
							<strong>Поддержать BlockEra</strong
							><span>Помогите развитию лаунчера — все функции останутся бесплатными.</span>
						</div>
						<b>Boosty</b>
					</a>
				</div>
...
const pendingItems = computed(() => items.value.filter((item) => item.updates > 0))
...
onMounted(async () => {
	...
	if (
		releaseNotes.some((note) => note.version === currentVersion.value) &&
...
.update-center-trigger {
	...
	border: 1px solid rgba(173, 92, 255, 0.18);
	border-radius: 13px;
	cursor: pointer;
}
.update-center-trigger:hover {
	background: rgba(145, 63, 224, 0.14);
	border-color: rgba(183, 103, 255, 0.4);
}
.update-center-trigger svg {
	width: 19px;
}
.update-center-trigger > span {
	position: absolute;
	min-width: 17px;
	height: 17px;
	right: -5px;
...
.update-center-popover > header div {
	display: flex;
	flex-direction: column;
}
.update-center-popover > header span,
.release-note-heading span {
	color: #b96dff;
	font-size: 9px;
	font-weight: 850;
	letter-spacing: 0.13em;
}
.update-center-popover h3 {
	margin: 3px 0 0;
	font-size: 18px;
}
.update-center-popover button {
	font: inherit;
	color: inherit;
	border: 0;
	cursor: pointer;
}
.update-center-popover > header button {
	width: 32px;
	height: 32px;
	display: grid;
	place-items: center;
...
.update-center-popover > header svg {
	width: 16px;
}
.update-tabs {
	display: grid;
...
.update-tabs button.active {
	color: #f2e8ff;
	background: rgba(145, 59, 230, 0.2);
	box-shadow: inset 0 0 0 1px rgba(190, 111, 255, 0.2);
}
.update-tab-panel {
	margin-top: 11px;
}
...
.update-summary {
	background: linear-gradient(135deg, rgba(141, 57, 231, 0.24), rgba(89, 31, 158, 0.1));
	border-color: rgba(190, 111, 255, 0.32);
}
.launcher-update-copy {
	min-width: 0;
	display: flex;
	flex-direction: column;
}
.launcher-update-copy > span {
	color: #bd76ff;
	font-size: 9px;
	font-weight: 850;
	letter-spacing: 0.12em;
}
.launcher-update-copy strong {
	margin-top: 3px;
	font-size: 13px;
}
.launcher-update-copy small {
	max-width: 260px;
	margin-top: 3px;
	overflow: hidden;
	color: #9ba1ae;
...
.install-launcher-update {
	...
	background: linear-gradient(135deg, #973ef0, #6822c9);
	border-radius: 9px;
	font-size: 10px !important;
	font-weight: 800 !important;
}
.install-launcher-update svg {
	width: 14px;
}
.spin {
...
.update-summary {
	...
	display: flex;
	flex-direction: column;
	border: 1px solid rgba(178, 95, 255, 0.17);
	border-radius: 12px;
}
...
.update-count {
	...
	color: #e4c9ff;
	text-align: center;
	background: rgba(143, 60, 226, 0.18);
	border-radius: 8px;
	font-size: 10px;
	font-weight: 850;
}
.update-instance-row > button {
	padding: 7px 9px;
	color: #dcbcff;
	background: rgba(145, 59, 230, 0.14);
	border-radius: 8px;
	font-size: 10px;
	font-weight: 750;
}
...
.update-tab-panel > footer button {
	padding: 9px 11px;
	display: flex;
	align-items: center;
	gap: 6px;
...
.update-tab-panel > footer svg {
	width: 14px;
}
.update-tab-panel > footer .update-all {
	color: white;
	background: linear-gradient(135deg, #9139ed, #6721c8);
}
.update-center-popover button:disabled {
	opacity: 0.5;
	cursor: wait;
}
.update-error {
...
.release-note-heading div {
	display: flex;
	flex-direction: column;
}
.release-note-heading h4 {
	margin: 2px 0 0;
	font-size: 15px;
}
...
.boosty-card {
	...
	gap: 12px;
	color: inherit;
	background: linear-gradient(135deg, rgba(145, 57, 237, 0.24), rgba(103, 33, 200, 0.12));
	border: 1px solid rgba(196, 130, 255, 0.26);
	border-radius: 13px;
